...
class OnlineCoursePlatformImpl extends OnlineCoursePlatform {
   ...
   * @return A sequence of courses in that category.
   */
  override def findCoursesByCategory(category: String): Sequence[Course] = courseList.filter(c => c.category == category)
   ...
   * @return An Optional containing the course if found, otherwise Optional.empty.
   */
  override def getCourse(courseId: String): Optional[Course] = courseList.find(c => c.courseId == courseId)
   ...
   * @return true if the course exists, false otherwise.
   */
  override def isCourseAvailable(courseId: String): Boolean = !courseList.find(c => c.courseId == courseId).isEmpty
